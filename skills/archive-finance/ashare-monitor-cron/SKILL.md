...
### 时间穿越审计后的报告元信息最小补丁
当用户只要求给收盘摘要/开盘表/持仓候选分析增加“数据时间信息”时，按最小补丁处理：
- 只改被点名的报告脚本，不顺手改其他报告链路。
- 不改 cron、数据库结构、候选生成、A/B/C 分层、scoreboard 写入或 DB latest 查询，除非用户明确要求。
- 无 git 仓库时先创建时间戳备份，最终回复列出备份路径与回滚命令。
- 新增独立 `build_*_report_metadata(now=None, target_date=None, data_warnings=None, capture_info=None)` 小函数，返回 Markdown，支持测试传入时间。
- 元信息块只插入在报告标题之后、原第一个正文标题之前，保持后续章节顺序不变。
- 新增针对元信息内容与插入位置的测试，并回归邻近报告元信息/asof 测试。

详见参考：`references/time-travel-audit-minimal-report-metadata.md`。
